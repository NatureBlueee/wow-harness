...
wow-harness 是 [Claude Code](https://docs.anthropic.com/en/docs/claude-code) 的治理层。它让 AI agent 变得足够可靠——你定方向，AI 交付，工作真的能落地。靠的不是提示词里多写几句"请认真做"，而是机械化的审查门禁、完成验证和工具隔离。
...
21 个 hook 覆盖 9 个生命周期阶段。不是事后审查，是实时拦截：

```
SessionStart       →  加载上下文、重置风险状态、展示可用工具
UserPromptSubmit   →  在消息发出时注入上下文片段
PreToolUse         →  拦截危险部署、门控审查 agent、净化读取内容
PostToolUse        →  编辑时路由上下文、检测循环、追踪风险
PostToolUseFailure →  分析工具失败、记录错误模式
PreCompact         →  压缩前注入最小化状态指针
Stop               →  验证是否存在完成候选品（transcript × git diff）
SessionEnd         →  反思、分析轨迹、持久化进度
WorktreeCreate     →  强制 H 系列施工隔离（H0.1）
```

Hook 通过 `run-py.sh` 桥接交付，同一套 Python 逻辑可在 Claude Code、
Codex、Cursor、OpenCode 运行，无需重复实现。
